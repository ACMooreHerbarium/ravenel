...
class SearchResults
{
	...
	private function splitData($records)
	{
		if(is_array($records) || is_object($records))
		{
			foreach($records as $record)
			{
				...
				$image = $record["pointer"] < 3000 ? 'https://digitalcollections.clemson.edu/utils/getthumbnail/collection/rvl/id/' : 'https://digital.tcl.sc.edu/utils/getthumbnail/collection/rav/id/';
	...
	*/
	...
	/*	utf8_encode and utf8_decode functions are deprecated in PHP 8.2,
		and will be removed in PHP 9.0 because these functions are misleadingly named,
		and are prone to unexpected and undesired results that emit no warnings or errors.
		Must modify utf8_encode function to something that works. Comment out and replace.
	*/
		//return utf8_encode($retVal);
		return mb_convert_encoding($retVal, "UTF-8", mb_detect_encoding($retVal));
