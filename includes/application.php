...
class Application {
	...
	*/
	public function getManuscriptCompoundObjectInfo($pointer)
	{
		return LINK_USC_S17.LINK_USC_GET_COMPOUND_OBJ.trim($pointer)."/json"; //  <- this is the correct thing!!!
	}
	...
	*/
	public function highlightSearchTerm($result, $search)
	{
		...
		//TODO::Fix this logic is actually broken, searching for 'high high' will break things because we find 'high' in 'highlight'
		if($result != "" && $search != "")
		{
			foreach($explodedSearch as $item)
			{
				...
				while(stripos($result,$item) !== false)
				{
					...
					$temp .= substr($result, 0, stripos($result, $item));
					$temp .= "<span class=\"highlight\">".$item."</span>";

					$result = substr($result,stripos($result, $item) + strlen($item)); // remove what we've added
				}

				$result = $temp.$result; // this should be the rest of the string with no instances of item at this point
			}
		}
	...
	*/
	...
	{
		// Not validating CAPTCHA for now - the domain key is incorrect
		// since the site was migrated to plantsandplanter.org.
		// - Joshua Nelson 2019-09-19
